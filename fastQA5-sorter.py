...
- minimum mean quality of a read to keep ('-q' option): 15 (Phred33);
...
min_ph33_qual = 15 # minimum mean read quality to keep
...
if not min_qlen is None:
    printl(" - Minimum length of a read to keep: {};".format(min_qlen))
# end if
...
def kernel(fpaths, srt_func):
    ...
    res_stats = srt_func(fpaths)

    if not (res_stats[0] == 0 and res_stats[1] == 0):
        if parallel:
            with inc_val_lock:
                inc_val.value += len(fpaths)
            # end if
        else:
            inc_val += 1
        # end if
