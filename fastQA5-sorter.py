...
try:
    opts, args = getopt.gnu_getopt(argv[1:], "hvr:d:o:s:q:m:f", ["help", "version", "taxannot-resdir=", "indir=", "outdir=",
        "sorting-sensitivity=", "min-ph33-qual=", "min-seq-len=", "verify-fast5"])
...
for fpath in QA5_list:
    ...
    if not verify_fast5 and fpath.endswith("fast5"):
...
'{}'\n  basecaling has been processed by 'prober.py' and 'barapost.py'.
...
Please, remove extra files and leave only one, which contains actual taxononic annotation info.""".format(taxann_dir))
        ...
        for f5_path in fast5_list:
            ...
            for i, read_name in enumerate(f5_file):
            ...
            while len(readids_to_seek) > 0:
                ...
                first_readid = intern(readids_to_seek[0])
                                ...
                                if readid in readids_in_tsv:
                                    if not first:
                                        try:
                                            ...
                                            second_ind_dict[tsv_taxann_fpath] = [readid]
                                        # end try
                                    readids_to_seek.remove(readid) # just for check
                                # end if
                            # end for
                            first = False
                            break
