...
def kernel(QA5_list):
    ...
    for fq_fa_path in QA5_list:
        ...
        if not fq_fa_path.endswith(".fast5"):
            buff_res = sort_fastqa_file(fq_fa_path)
            seqs_pass += buff_res[0]
            seqs_fail += buff_res[1]
        else:
            buff_res = sort_fast5_file(fq_fa_path)
            seqs_pass += buff_res[0]
            seqs_fail += buff_res[1]
        # end if
...
if n_thr == 1:
    ...
    seqs_pass, seqs_fail = kernel(QA5_list)

else:
    inc_val = mp.Value('i', 0)
    get_inc_val = lambda: inc_val.value # merely get this value (1 thread)
    ...
    pool = mp.Pool(n_thr, initializer=init_paral_sorting, initargs=(write_lock, inc_val, inc_val_lock))
    result = pool.starmap(kernel, [(QA5_sublist,) for QA5_sublist in spread_files_equally(QA5_list, n_thr)])

    pool.close()
    pool.join()

    seqs_pass = sum(map(lambda t: t[0], result))
    seqs_fail = sum(map(lambda t: t[1], result))
# end if
...
# If there is need to gzip FASTQ or FASTA files in output directory, proceed gzipping
...
if n_thr == 1:
    ...
    pool = mp.Pool(n_thr)
    pool.starmap(gzip_func, [(fastqa_sublist,) for fastqa_sublist in spread_files_equally(fastqa_res_files, n_thr)])
    pool.close()
    pool.join() 
# end if
