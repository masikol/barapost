...
EXAMPLES:\n
  ...
  4. Process all FASTQ and FASTA files in directory named 'dir_with_seqs'. Sort by genus.
     ...
     Results of "barapost.py" work are in directory 'prober_outdir':\n
     ./fastQA5-sorter.py -d dir_with_seqs -o outdir -r prober_outdir/ -s genus
...
if untwist_fast5:
    ...
    index_name = "fast5_to_fastq_idx" # "primary"
    second_index_name = index_name+"_secondary" # "secondary"
    ...
    def map_reads_2_taxann(QA5_list):
        ...
        It creates two index DBM files: so called "primary" and "secondary".

        "Primary" one maps whole FAST5 files to TSV files according to information
            where taxonomic annotation info for thr FIRST read in FAST5 file is stored.
        Assumming that only names of FAST5-FASTQ files are messed up, this index file
            can be enough to untwist the basecaling clew (and the file is small).
        It's structure:

        {
            <path_to_FAST5_1>: <path_to_TSV_1>,
            <path_to_FAST5_2>: <path_to_TSV_2>,
            ...
            <path_to_FAST5_N>: <path_to_TSV_N>
        }

        If reads from one FAST5 are spread between several FASTQ (and hence, TSV-taxann) files,
            then the "secondary" index comes to us.
        It's structure allows to minimize times needed to read plain (i.e. sequential access) TSV files.
        Well, structure of "secondary" index is following:
...
Please, remove extra files and leave only one, which contains actual taxononic annotation info.""".format(taxann_dir))
        ...
        # index_read2tsv = open_shelve( os.path.join(index_dirpath, index_name), 'n' )
        index_f5_2_tsv = open_shelve( os.path.join(index_dirpath, second_index_name), 'n' )
        ...
        for f5_path in fast5_list:
            ...
            second_ind_dict = dict() # dictionary for "secondary" index
            ...
            # This variable indicates if a "secondary" index is needed.
            # If is is True, FAST5-to-TSV mapping is enough and "secondary" index is not necessary.
            first = True

            # Until taxonomic annotation for all reads is found
            while len(readids_to_seek) > 0:
                ...
                for tsv_taxann_fpath in tsv_taxann_lst:
                    ...
                    # first_readid = intern(readids_to_seek[0]) # get first-best read ID from FAST5
                    ...
                    with open(tsv_taxann_fpath, 'r') as taxann_file:
                        ...
                        # If TSV containing first read is found -- check if all other reads
                        #    from this FAST5 are mentioned in current TSV-taxann
                        # if first_readid in readids_in_tsv:

                            # Write a record to "primary" index file
                            # index_read2tsv[f5_path] = os.path.abspath(tsv_taxann_fpath)
                            # readids_to_seek.remove(first_readid) # exclude this read ID from list
                        ...
                        for readid in reversed(readids_to_seek):
                            if readid in readids_in_tsv:
                                # If not first -- write data to dict (and to "secondary" inde later)
                                try:
                            ...
                            # end if
                        # end for
                        # first = False # at this point first-best TSV is checked
                        # break
                        # end if
                    # end with
                # end for
    ...
    def sort_fast5_file(f5_path):
        ...
        readids_to_seek = list() # list of not-sorted-yet read IDs
        ...
        # Retrieve first-best TSV
        # index_read2tsv = open_shelve( os.path.join(index_dirpath, index_name), 'r')
        ...
        # Process reads that are present in first-best TSV
        ...
        #     except KeyError:
        #         j += 1 # everything is alredy checked
        #     else:
        #         q_len = SeqLength(q_len)
        #         if q_len < min_qlen or (ph33_qual != '-' and ph33_qual < min_ph33_qual):
        #             # Get name of result FASTQ file to write this read in
        ...
        # Walk through the "secondary" index
        # if len(readids_to_seek) != 0:
        index_f5_2_tsv = open_shelve( os.path.join(index_dirpath, second_index_name), 'r' )
        ...
        for tsv_path in index_f5_2_tsv[f5_path].keys():
            ...
            for read_name in read_names:
                hit_name, ph33_qual, q_len = resfile_lines[read_name[5:]]
                q_len = SeqLength(q_len)
                if q_len < min_qlen or (ph33_qual != '-' and ph33_qual < min_ph33_qual):
                    # Get name of result FASTQ file to write this read in
                    copy_read_f5_2_f5(from_f5, read_name, trash_fpath)
