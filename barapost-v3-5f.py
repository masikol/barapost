...
DESCRIPTION:\n
 ...
 'barapost-v3.5a.py' is meant to be used just after 'prober-v1-12f.py'.
...
of work of 'prober-v1-12f.py' script, build an indexed local database which consists of
...
OPTIONS:\n
    ...
    -r (--prober-result-dir) --- result directory generated by script 'prober-v1-12f.py'
        This is directory specified to 'prober-v1-12f.py' by '-o' option.
        If you omit 'prober-v1-12f.py' and use your own FASTA files
        to create a database, this directory may not exist before start of 'barapost-v3-5f.py'
...
EXAMPLES:\n
  ...
  6. Process file named 'some_reads.fastq'. This file has been already processed by "prober-v1-12f.py".
     Results of "prober-v1-12f.py" work are in directory 'prober_outdir'.
     Sequence from file 'my_own_sequence.fasta' will be included to the database.
     Packet size is 50 sequences. Launch 4 threads.\n
     ./barapost-v3-5f.py some_reads.fastq -p 50 -l my_own_sequence.fasta -t 4 -r prober_outdir
"""
from sys import argv
import getopt

try:
    opts, args = getopt.gnu_getopt(argv[1:], "hd:p:a:r:l:t:",
...
prober_res_dir = "barapost_result" # default
...
try:
    seem_like_acc_files = filter(is_acc_file, os.listdir(prober_res_dir))
except FileNotFoundError:
    # If 'prober_res_dir' does not exist-- obviously there is no accession file
    like_acc_num = 0
else:
...
if like_acc_num == 1:
    acc_fpath = os.path.join(prober_res_dir, next(filter(is_acc_file, os.listdir(prober_res_dir)))) # form path to accession file
    ...
    for f in filter(is_acc_file, os.listdir(prober_res_dir)):
        printl("  '{}'".format(f))
    # end for
...
# end if
    ...
    if prober_res_dir == "barapost_result":
        print("""Maybe, output directory generated by 'prober-v1-12f.py' hasn't been named 'barapost_result'
...
logfile_path = os.path.join(prober_res_dir, "barapost_log_{}{}".format(strftime("%Y-%m-%d_%H-%M-%S", localtime(start_time)), log_ext))
...
logfile.write((" |=== barapost-v3-5f.py ===|\n\n"))
...
def retrieve_fastas_by_gi(gi_list, db_dir):
    ...
    while error:
        try:
            ...
            printl( str(err) )
            printl("'barapost-v3.5a.py' will try again in 30 seconds")
...
def launch_blastn(packet, blast_algorithm):
    ...
    # In order to provide full cli-interface compatibility with 'prober-v1-12f.py' I will merely change values here.
...
def get_curr_res_dpath(fq_fa_path, prober_res_dir):
    """
    Function configures and returns the path to result directory for particular FASTA or FASTQ file.
    Result directory is nested in 'prober_res_dir' and is named according to name of FASTA/FASTQ file.
    ...
    :type fq_fa_path: str;
    :param prober_res_dir: path to directory with results of 'prober-v1-12f.py';
    :type prober_res_dir: str;
...
def configure_acc_dict(acc_fpath):
    """
    Fucntion couffigures accession dictionary according to accessin file generated by 'prober-v1-12f.py':
       keys are accessions, values are tuples of the following format:
        (<GI_number>, <sequence_name_aka_definition>).

    :param acc_fpath: path to accession file generated by 'prober-v1-12f.py';
    :type acc_fpath: str;
...
def process_multiple_files(fq_fa_list, parallel=False):
    ...
    for i, fq_fa_path in enumerate(fq_fa_list):
        ...
        new_dpath = get_curr_res_dpath(fq_fa_path, prober_res_dir)
...
queries_tmp_dir = os.path.join(prober_res_dir, "queries-tmp")
