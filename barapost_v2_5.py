...
try:#{
    opts, args = getopt.getopt(argv[1:], "hf:d:o:p:a:r:l:",
...
def retrieve_fastas_by_gi(gi_list, db_dir):#{
    ...
    print("\nDownloading sequences for local database building...")
    ...
    if not len(your_own_fasta_lst) == 0:#{
        ...
        with open(local_fasta, 'a') as fasta_db:#{
            for own_fasta_path in your_own_fasta_lst:#{
                print("Adding '{}' to database...".format(os.path.basename(own_fasta_path)))

                how_to_open = OPEN_FUNCS[ is_gzipped(own_fasta_path) ]
                with how_to_open(own_fasta_path) as fasta_file:#{
                    fasta_db.write(fasta_file.read().strip()+'\n') # 'strip' is necessary to avoid blank lines between records
                #}
            #}
        #}
    #}

    print("Creating database...", end="") # end="" in order not to print too many blank lines
