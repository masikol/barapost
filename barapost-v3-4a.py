...
is_acc_file = lambda f: False if re_search(r".*{}probe_acc_list.tsv".format(os.sep), os.listdir(prober_res_dir)) is None else True
if len( list(filter(is_acc_file, os.listdir(prober_res_dir))) ) > 1:
    ...
    print("Please, remove excess files and leave only one of them.")
    platf_depend_exit(1)
# end if
...
logfile.write((" |=== barapost-v3.4a.py ===|\n\n"))
...
def retrieve_fastas_by_gi(gi_list, db_dir):
    ...
    while error:
        try:
            ...
            printl( str(err) )
            printl("'barapost-v3.4a.py' will try again in 30 seconds")
