...
1. **prober-v1-10a.py** -- this program sends several sequences (aka probing batch) to NCBI BLAST server in order to determine what taxonomic units are present in data set. "prober-v1-10a.py" saves information about the best hit of each sequence from probing batch.
Process all sequences in this way takes too much time, what leads us to "barapost-v3-4a.py".

2. **barapost-v3-4a.py** -- this program firstly downloads best hits discovered by "prober-v1-10a.py" from Genbank, then uses these downloaded sequences to build a database on your local machine and finally aligns the rest of data set against builded database. Database building and "BLASTing" is performed by using "BLAST+" toolkit.
Results are written in TSV file named `...results.tsv`.

3. **fastQA-sorter-v2-8a.py** -- this program performs sorting (dividing into separate files) of your data set according to results of "prober-v1-10a.py" and "barapost-v3-4a.py"
...
Version 1.10.a; 04.10.2019 edition;
...
FASTQ files processed by this program are meant to be processed afterwards by "barapost-v3-4a.py".

If you have your own FASTA files that can be used as database to blast against, you can omit "prober-v1-10a.py" step and go to "barapost-v3-4a.py" (see `-l` option in "barapost-v3-4a.py" description).
...
Note for Windows users: `./prober-v1-10a.py` won't work on Windows -- type `python prober-v1-10a.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x prober-v1-10a.py`) and run it as it is shown in examples below.
...
Version 3.4.a; 04.10.2019 edition;
...
"barapost-v3-4a.py" is meant to be used just after 'prober-v1-10a.py'.

"barapost-v3-4a.py" downloads records-hits from Genbank according to results (`...acc_list.tsv`)
of work of "prober-v1-10a.py", builds an indexed local database which consists of
...
"barapost-v3-4a.py" writes it's results in the same TSV file as "prober-v1-10a.py" does.
...
```
    ...
    -r (--prober-result-dir) --- result directory genearted by program "prober-v1-10a.py"
        This is directory specified to "prober-v1-10a.py" by '-o' option.
        Default value is "prober_result", since it is the default name of
        output directory generated by "prober-v1-10a.py".
    ...
    -l (--local-fasta-to-db) --- your own FASTA file that will be added to downloaded database
        or used instead of it if you omit 'prober-v1-10a.py' step;
    ...
    -o (--outdir) --- output directory. Can be used only if '-l' option is specified.
        ...
        accession file generated by "prober-v1-10a.py". If you omit "prober-v1-10a.py" stage and specify your own FASTA
...
1. Besides using `-l` option, you can specify your own FASTA files using accession TSV file generated by "prober-v1-10a.py". To do this, just write your FASTA file's path to this TSV file in new line.
...
`./prober-v1-10.py some_my_fasta*`
  ...
  3. Process one FASTQ file with default settings.
     File `reads.fastq` has been already processed by "prober-v1-10a.py".
     Results of "prober-v1-10a.py" work are in directory `prober_outdir`:
...
`./barapost-v3-4a.py reads.fastq -r prober_outdir`
  ...
  4. Process FASTQ file and FASTA file with discoMegablast, packet size of 100 sequences.
     Files `reads.fastq.gz` and `another_sequences.fasta` have been already processed by "prober-v1-10a.py".
     Results of "prober-v1-10a.py" work are in directory `prober_outdir`:
...
`./barapost-v3-4a.py reads.fastq.gz another_sequences.fasta -a discoMegablast -p 100 -r prober_outdir`
  ...
  5. Process all FASTQ and FASTA files in directory named `some_dir`.
    All these files have been already processed by "prober-v1-10a.py".
    Results of "prober-v1-10a.py" work are in directory `prober_outdir`:

`/barapost-v3-4a.py -d some_dir -r prober_outdir`

  6. Process file named `some_reads.fastq`. This file has been already processed by "prober-v1-10a.py".
     Results of "prober-v1-10a.py" work are in directory `prober_outdir`. Sequence from file `my_own_sequence.fasta` will be included to the database.
...
Version 2.8.a; 05.10.2019 edition;
...
```
    ...
    -q (--min-ph33-qual) --- minimum Phred33 quality to keep in sorted files.
        Reads of lower quality will be omitted.
```
...
`./fastQA-sorter-v2-8a.py -f reads.fastq.gz -r prober_outdir/`
...
`./fastQA-sorter-v2-8a.py -f reads_1.fastq.gz -f some_sequences_2.fasta -o outdir -r prober_outdir/`
...
`./prober-v1-10a.py && ./barapost-v3-4a.py && ./fastQA-sorter-v2-8a.py`

2. You can try this programs on test dataset named `some_reads.fastq` (there are 4 reads):

`./prober-v1-10a.py -f some_reads.fastq -o some_outdir -g Escherichia,561+viruses,10239 -p 2 -b 2`

`./barapost-v3-4a.py -f some_reads.fastq -r some_outdir`

`./fastQA-sorter-v2-8a.py -f some_reads.fastq -r some_outdir -o some_sorted_reads`
