...
2. **barapost-v3-5c.py** -- this script firstly downloads best hits discovered by "prober-v1-12e.py" from Genbank, then uses these downloaded sequences to build a database on your local machine and finally aligns the rest of data set against builded database. Database building and "BLASTing" is performed by using "BLAST+" toolkit.
Results are written in TSV file named `...results.tsv`.

3. **fastQA5-sorter-v3-1b.py** -- this script performs sorting (dividing into separate files) of your data set according to results of "prober-v1-12e.py" and "barapost-v3-5c.py"
...
If you have your own FASTA files that can be used as database to blast against, you can omit "prober-v1-12e.py" step and go to "barapost-v3-5c.py" (see `-l` option in "barapost-v3-5c.py" description).
...
"barapost-v3-5c.py" is meant to be used just after 'prober-v1-12e.py'.
...
"barapost-v3-5c.py" writes it's results in the same TSV file as "prober-v1-12e.py" does.
...
Version 3.1.a; 2019.10.23 edition;

### DESCRIPTION:

**fastQA5-sorter-v3-1b.py** -- this script is designed for sorting (dividing into separate files) FASTQ and FASTA files processed by "barapost-v3-5c.py".
...
- Files that you want "fastQA5-sorter-v3-1b.py" to process should be specified as positional arguments (see EXAMPLE #2 below).
...
"fastQA_sorted.py" decides whether it is a SPAdes assembly file or a a5 one by looking at sequence IDs in these files. If ID is like `"NODE_1_length_245432_cov_23.5412"` -- probably it is SPAdes work. If ID is like `"scaffold_1"` -- it looks just like a5 output. "fastQA5-sorter-v3-1b.py" regards only the first sequnce ID in file.

### EXAMPLES:

Note for Windows users: `./fastQA5-sorter-v3-1b.py` won't work on Windows -- type `python fastQA5-sorter-v3-1b.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x fastQA5-sorter-v3-1b.py`) and run it as it is shown in examples below.
...
FAST5 files can be sorted by fastQA5-sorter-v3-1b.py.
...
Therefore, you can pass FAST5 files to "fastQA5-sorter-v3-1b.py" just as FASTQ or FASTA files and they will be sorted as well.
...
The short and reliable rule souds so: **do not rename your FASTQ and FAST5 files after basecalling** if you intend to sort these FAST5 files with "fastQA5-sorter-v3-1b.py".
...
- Experience tells that check string (more precisely, it's hash-like part before the underscore) has length of 41. Assuming that this situation can vary from case to case, "fastQA5-sorter-v3-1b.py" will match check string that contains hash-like part with length at least 30 characters (or more). Also I have not seen uppercase letters in such hash-like character sequences, but it is better to assume and forsee their existance.

Algorithm of finding taxonomical annotation infornation for a particular FAST5 file sorting:

1. "fastQA5-sorter-v3-1b.py" tries to match the pattern described above with regex. Exact pattern:
...
Sorting:
```
./fastQA5-sorter-v3-1b.py some_reads.fast5
```
...
`./prober-v1-12e.py && ./barapost-v3-5c.py && ./fastQA5-sorter-v3-1b.py`
...
`./fastQA5-sorter-v3-1b.py some_reads.fastq -r some_outdir -o some_sorted_reads`
