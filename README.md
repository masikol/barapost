...
1. **prober-v1-8a.py** -- this program sends several sequences (aka probing batch) to NCBI BLAST server in order to determine what taxonomic units are present in data set. "prober-v1-8a.py" saves information about the best hit of each sequence from probing batch.
Process all sequences in this way takes too much time, what leads us to "barapost-v3-1a.py".

2. **barapost-v3-1a.py** -- this program firstly downloads best hits discovered by "prober-v1-8a.py" from Genbank, then uses these downloaded sequences to build a database on your local machine and finally aligns the rest of data set against builded database. Database building and "BLASTing" is performed by using "BLAST+" toolkit.
Results are written in TSV file named `...results.tsv`.

3. **fastQA-sorter-v2-6a.py** -- this program performs sorting (dividing into separate files) of your data set according to results of "prober-v1-8a.py" and "barapost-v3-1a.py"
...
**prober-v1-8a.py** -- this program is designed for determinating the taxonomic position
...
on your local machine by "barapost-v3-1a.py".
...
1) There is a file named `...acc_list.tsv`. It contains accessions and names of Genbank records that
    can be used for building a database on your local machine by "barapost-v3-1a.py".

2) There is a file named `...result.tsv`. It contains full result of "BLASTing".
    Results of barapost-v3-1a.py's work will be appended to this file.

FASTQ files processed by this program are meant to be processed afterwards by "barapost-v3-1a.py".

If you have your own FASTA files that can be used as database to blast against, you can omit "prober-v1-8a.py" step and go to "barapost-v3-1a.py" (see `-l` option in "barapost-v3-1a.py" description).
...
It means that you should not process all your data by "prober-v1-8a.py' -- it would take long time.

Instead of this you should process some sequences by "prober-v1-8a.py" -- it will determine,
what Genbank records (genomes, if you want) are present in your data and then go to "barapost-v3-1a.py".

"barapost-v3-1a.py" will process the rest of you sequences in the same way like "prober-v1-8a.py", but on your local computer.
"barapost-v3-1a.py" uses 'BLAST+' toolkit for this purpose. It will be much faster.

Obviously, a probing batch cannot cover all variety of a data set,
so some sequences can be recognized as "unknown" while processing by "barapost-v3-1a.py".
But you always can run "prober-v1-8a.py" again on "unknown" sequences.
...
Note for Windows users: `./prober-v1-8a.py` won't work on Windows -- type `python prober-v1-8a.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x prober-v1-8a.py`) and run it as it is shown in examples below.
...
`./prober-v1-8a.py -f reads_1.fastq.gz -f some_sequences.fasta -a discoMegablast -p 100 -g Erwinia,551`
...
`./prober-v1-8a.py -d some_dir -g Escherichia,561+viruses,10239 -o outdir -b 300 -p 100`
...
**barapost-v3-1a.py** -- this program is designed for determinating the taxonomic position
of nucleotide sequences by "BLASTing" each of them with 'blastn' program from "BLAST+" toolkit
and regarding the best hit.

"barapost-v3-1a.py" is meant to be used just after 'prober-v1-8a.py'.

"barapost-v3-1a.py" downloads records-hits from Genbank according to results (`...acc_list.tsv`)
of work of "prober-v1-8a.py", builds an indexed local database which consists of
...
"barapost-v3-1a.py" writes it's results in the same TSV file as "prober-v1-8a.py" does.
...
```
    ...
    -r (--prober-result-dir) --- result directory genearted by program "prober-v1-8a.py"
        This is directory specified to "prober-v1-8a.py" by '-o' option.
        Default value is "prober_result", since it is the default name of
        output directory generated by "prober-v1-8a.py".
    ...
    -l (--local-fasta-to-db) --- your own FASTA file that will be added to downloaded database
        or used instead of it if you omit 'prober-v1-8a.py' step;

    -o (--outdir) --- output directory. Can be used only if '-l' option is specified.
        The reason is that results of 'barapost-v3-1a.py' should be written to the directory that contains
        accession file generated by "prober-v1-8a.py". If you omit "prober-v1-8a.py" stage and specify your own FASTA
...
1. Besides using `-l` option, you can specify your own FASTA files using accession TSV file generated by "prober-v1-8a.py". To do this, just write your FASTA file's path to this TSV file in new line.

2. "makeblastdb" utility from "BLAST+" toolkit considers first word (it separates words by spaces) of sequence ID in FASTA file as sequence accession. Naturally, duplicated accessions are not allowed. Therefore, in order to avoid this duplication, "barapost-v3-1a.py" uses modified sequence IDs of your own sequences in FASTA files while database creating. It adds custom accession number in the beginning of sequence IDs. This custom accessions have following format: OWN_SEQ_<N>, where <N> is an integer number. Actually, it is order number of this sequence (I mean order of adding to database). Do not worry: these modified sequence IDs are used only in database -- your own FASTA files will be kept intact.

3. If you include SPAdes or a5 assembly FASTA file in the database with "barapost-v3-1a.py", sequence IDs will be modified in a specific (i.e. *ad hoc*) way. If there are **more than one** assembly file generated by **one** assembler (e.g. two files named "contigs.fasta" generated by SPAdes), paths to these "contigs.fasta" files will be added to sequence IDs while database creation. So, sequence IDs will look like, e.g. for SPAdes:
...
Note for Windows users: `./barapost-v3-1a.py` won't work on Windows -- type `python barapost-v3-1a.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x barapost-v3-1a.py`) and run it as it is shown in examples below.
...
`./barapost-v3-1a.py`
  ...
  2. Process one FASTQ file with default settings.
     File `reads.fastq` has been already processed by "prober-v1-8a.py".
     Results of "prober-v1-8a.py" work are in directory `prober_outdir`:

`./barapost-v3-1a.py -f reads.fastq -r prober_outdir`

  3. Process FASTQ file and FASTA file with discoMegablast, packet size of 100 sequences.
     Files `reads.fastq.gz` and `another_sequences.fasta` have been already processed by "prober-v1-8a.py".
     Results of "prober-v1-8a.py" work are in directory `prober_outdir`:

`./barapost-v3-1a.py -f reads.fastq.gz -f another_sequences.fasta -a discoMegablast -p 100 -r prober_outdir`

  4. Process all FASTQ and FASTA files in directory named `some_dir`.
    All these files have been already processed by "prober-v1-8a.py".
    Results of "prober-v1-8a.py" work are in directory `prober_outdir`:

`/barapost-v3-1a.py -d some_dir -r prober_outdir`

  5. Process file named `some_reads.fastq`. This file has been already processed by "prober-v1-8a.py".
     Results of "prober-v1-8a.py" work are in directory `prober_outdir`. Sequence from file `my_own_sequence.fasta` will be included to the database.
     Packet size is 50 sequences. Launch 4 threads.

`./barapost-v3-1a.py -f some_reads.fastq -p 50 -l my_own_sequence.fasta -t 4 -r prober_outdir`


## 3. fastQA-sorter-v2-6a.py
...
**fastQA-sorter-v2-6a.py** -- this program is designed for sorting (dividing into separate files) FASTQ and FASTA files processed by "barapost-v3-1a.py".

"fastQA_sorter-v2-6.py" is meant to be used just after "barapost-v3-1a.py".
...
"fastQA_sorted.py" decides whether it is a SPAdes assembly file or a a5 one by looking at sequence IDs in these files. If ID is like `"NODE_1_length_245432_cov_23.5412"` -- probably it is SPAdes work. If ID is like `"scaffold_1"` -- it looks just like a5 output. "fastQA-sorter-v2-6a.py" regards only the first sequnce ID in file.


### EXAMPLES:

Note for Windows users: `./fastQA-sorter-v2-6a.py` won't work on Windows -- type `python fastQA-sorter-v2-6a.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x fastQA-sorter-v2-6a.py`) and run it as it is shown in examples below.

  1. Process all FASTA and FASTQ files in working directory with default settings:

`./fastQA-sorter-v2-6a.py`

  2. Process one FASTQ file with default settings.
     File `reads.fastq` has been already processed by "barapost-v3-1a.py".
     Results of "barapost-v3-1a.py" work are in directory `prober_outdir`:

`./fastQA-sorter-v2-6a.py -f reads.fastq.gz -r prober_outdir/`

  3. Process a FASTQ file and a FASTA file, place results in `outdir` directory.
     Files `reads.fastq.gz` and `another_sequences.fasta` have been already processed by "barapost-v3-1a.py".
     Results of "barapost-v3-1a.py" work are in directory `prober_outdir`:

`./fastQA-sorter-v2-6a.py -f reads_1.fastq.gz -f some_sequences_2.fasta -o outdir -r prober_outdir/`

  4. Process all FASTQ and FASTA files in directory named `dir_with_seqs`. Sort by genus.
     All these files have been already processed by "barapost-v3-1a.py".
     Results of "barapost-v3-1a.py" work are in directory `prober_outdir`:

`./fastQA-sorter-v2-6a.py -d dir_with_seqs -o outdir -r prober_outdir/ -s genus`
...
`./prober-v1-8a.py && ./barapost-v3-1a.py && ./fastQA-sorter-v2-6a.py`

2. You can try this programs on test dataset named `some_reads.fastq` (there are 4 reads):

`./prober-v1-8a.py -f some_reads.fastq -o some_outdir -g Escherichia,561+viruses,10239 -p 2 -b 2`

`./barapost-v3-1a.py -f some_reads.fastq -r some_outdir`

`./fastQA-sorter-v2-6a.py -f some_reads.fastq -r some_outdir -o some_sorted_reads`
