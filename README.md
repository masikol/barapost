...
Process all sequences in this way takes too much time, what leads us to "barapost-v3-1b.py".

2. **barapost-v3-1b.py** -- this program firstly downloads best hits discovered by "prober-v1-8b.py" from Genbank, then uses these downloaded sequences to build a database on your local machine and finally aligns the rest of data set against builded database. Database building and "BLASTing" is performed by using "BLAST+" toolkit.
Results are written in TSV file named `...results.tsv`.

3. **fastQA-sorter-v2-6b.py** -- this program performs sorting (dividing into separate files) of your data set according to results of "prober-v1-8b.py" and "barapost-v3-1b.py"
...
on your local machine by "barapost-v3-1b.py".
...
1) There is a file named `...acc_list.tsv`. It contains accessions and names of Genbank records that
    can be used for building a database on your local machine by "barapost-v3-1b.py".

2) There is a file named `...result.tsv`. It contains full result of "BLASTing".
    Results of barapost-v3-1b.py's work will be appended to this file.

FASTQ files processed by this program are meant to be processed afterwards by "barapost-v3-1b.py".

If you have your own FASTA files that can be used as database to blast against, you can omit "prober-v1-8b.py" step and go to "barapost-v3-1b.py" (see `-l` option in "barapost-v3-1b.py" description).
...
what Genbank records (genomes, if you want) are present in your data and then go to "barapost-v3-1b.py".

"barapost-v3-1b.py" will process the rest of you sequences in the same way like "prober-v1-8b.py", but on your local computer.
"barapost-v3-1b.py" uses 'BLAST+' toolkit for this purpose. It will be much faster.

Obviously, a probing batch cannot cover all variety of a data set,
so some sequences can be recognized as "unknown" while processing by "barapost-v3-1b.py".
...
**barapost-v3-1b.py** -- this program is designed for determinating the taxonomic position
of nucleotide sequences by "BLASTing" each of them with 'blastn' program from "BLAST+" toolkit
and regarding the best hit.

"barapost-v3-1b.py" is meant to be used just after 'prober-v1-8b.py'.

"barapost-v3-1b.py" downloads records-hits from Genbank according to results (`...acc_list.tsv`)
...
"barapost-v3-1b.py" writes it's results in the same TSV file as "prober-v1-8b.py" does.
...
```
    ...
    -o (--outdir) --- output directory. Can be used only if '-l' option is specified.
        The reason is that results of 'barapost-v3-1b.py' should be written to the directory that contains
...
Note for Windows users: `./barapost-v3-1b.py` won't work on Windows -- type `python barapost-v3-1b.py` instead.

Sure, you can do the same thing on Unix-like systems, but you might face problems with path completions if you call Python interpreter explicitly. Therefore I recommend to make .py-file executable (by running `chmod +x barapost-v3-1b.py`) and run it as it is shown in examples below.
...
`./barapost-v3-1b.py -f reads.fastq -r prober_outdir`
...
`./barapost-v3-1b.py -f reads.fastq.gz -f another_sequences.fasta -a discoMegablast -p 100 -r prober_outdir`
...
`/barapost-v3-1b.py -d some_dir -r prober_outdir`
...
`./barapost-v3-1b.py -f some_reads.fastq -p 50 -l my_own_sequence.fasta -t 4 -r prober_outdir`
...
**fastQA-sorter-v2-6b.py** -- this program is designed for sorting (dividing into separate files) FASTQ and FASTA files processed by "barapost-v3-1b.py".

"fastQA_sorter-v2-6.py" is meant to be used just after "barapost-v3-1b.py".
...
`./fastQA-sorter-v2-6b.py`
  ...
  2. Process one FASTQ file with default settings.
     File `reads.fastq` has been already processed by "barapost-v3-1b.py".
     Results of "barapost-v3-1b.py" work are in directory `prober_outdir`:
...
`./fastQA-sorter-v2-6b.py -f reads.fastq.gz -r prober_outdir/`
  ...
  3. Process a FASTQ file and a FASTA file, place results in `outdir` directory.
     Files `reads.fastq.gz` and `another_sequences.fasta` have been already processed by "barapost-v3-1b.py".
     Results of "barapost-v3-1b.py" work are in directory `prober_outdir`:
...
`./fastQA-sorter-v2-6b.py -f reads_1.fastq.gz -f some_sequences_2.fasta -o outdir -r prober_outdir/`
  ...
  4. Process all FASTQ and FASTA files in directory named `dir_with_seqs`. Sort by genus.
     All these files have been already processed by "barapost-v3-1b.py".
     Results of "barapost-v3-1b.py" work are in directory `prober_outdir`:
...
`./prober-v1-8b.py && ./barapost-v3-1b.py && ./fastQA-sorter-v2-6b.py`
...
`./barapost-v3-1b.py -f some_reads.fastq -r some_outdir`
