...
help_msg = """
DESCRIPTION:\n
 fastq_forter.py -- script designed for sorting FASTQ files processed by barapost.py.
...
OPTIONS (* means mandatory option):\n
  ...
  * -b (--barapost-result) --- result TSV file generated by barapost.py;
    ...
    -s (--sorting-sensitivity) --- sorting sensitivity, e.i. the lowest taxonomy rank that will be used in names of resut files;
...
for opt, arg in opts:#{
    ...
    if opt in ("-f", "--infile"):#{
        ...
        if not is_fastq_or_gz(arg):#{
            print(" !! - Warning: file '{}' is not '.fastq' and not '.fastq.gz'!".format(arg))
            platf_depend_exit(1)
        #}
