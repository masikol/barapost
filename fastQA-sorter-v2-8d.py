...
is_fq_or_fa = lambda f: True if not re_search(r".*\.(m)?f(ast)?(a|q|5)(\.gz)?$", f) is None else False
...
if len(fq_fa_list) == 0:
    ...
    # If input directory was not specified -- look for FASTQ and FASTA files in current directory
        ...
        fq_fa_list.extend(list(filter( is_fq_or_fa, glob("{}{}*".format(os.getcwd(), os.sep)) )))
    # end if
# end if
...
def sort_fast5_file(f5_path):
    ...
    for read_name in from_f5:
        ...
        try:
            hit_name, ph33_qual = resfile_lines[read_name[5:]] # find hit corresponding to this sequence
        except KeyError:
