...
help_msg = """
DESCRIPTION:\n
fastQA-sorter-v2-8c.py -- this program is designed for sorting (dividing into separate files)
    FASTQ and FASTA files processed by "barapost-v3-5a.py".\n
...
----------------------------------------------------------\n
Files that you want 'fastQA-sorter-v2-8c.py' to process should be
...
prober_res_dir = "prober_result" # path to result TSV file generated by barapost-v3-5a.py
...
if len(fq_fa_list) == 0:
    ...
    if not indir_path is None:
        print(err_fmt("no input FASTQ or FASTA files specified or there is no FASTQ or FASTA files in the input directory.\n"))
        platf_depend_exit(1)
    ...
    # If input directory was not specified -- look for FASTQ and FASTA files in current directory
        ...
        fq_fa_list.extend(currdir_fastqs)
    # end if
# end if
...
logfile.write((" |=== fastQA-sorter-v2-8c.py ===|\n\n"))
...
if len( list( filter(is_fq_or_fa, os.listdir(outdir_path)) ) ) != 0:
    printl("Attention! Ouput directory '{}' is not empty!".format(outdir_path))
    printl("List of sequence-containing files in it:")
    for i, file in enumerate(filter(is_fq_or_fa, os.listdir(outdir_path))):
    ...
    or enter 'r' to rename old directory and to write current results to a new one:>>""")
        ...
        if reply == "":
            ...
            for file in filter(is_fq_or_fa, os.listdir(outdir_path)):
...
# |=== Function for configuring dictionary containing information generated by barapost-v3-5a.py ===|
...
def get_res_tsv_fpath(new_dpath):
    ...
    file '{}...' have been processed by 'barapost-v3-5a.py'""".format(os.path.basename(new_dpath)))
    ...
    with how_to_open(fq_fa_path) as source_fastq_file:
        ...
        for i in range(num_reads):
            ...
            try:
                ...
                printl("Make sure that this read has been already processed by 'prober.py' and 'barapost-v3-5a.py'.")
                continue
